...
Staged-run steps (user) — steps 1–2 of the original list were completed in-session after the editor recovered (the API Updater dialog was the wedge; declined): confirmatory suite re-run on the restored manifest is **green (4/4 + 1 ignored)**, and `N2Bootstrap.unity` (YAML-authored Bootstrap copy, starter swapped to `N2Starter`) imported and validated — `N2Starter` present on `Stage3Rig`, zero missing scripts. Remaining:
1. Open `N2Bootstrap` in the main editor (already open at session end) AND in the ParrelSync clone; press Play in the main editor first (auto-hosts), then in the clone (auto-joins as guest). Watch the guest load `Layer`.
2. Hand both consoles' `[TETHER]` streams to Claude for the log review (owner doc §9).

Note: with the restored (unpinned) manifest the sandbox resolves transport **1.5.0** / collections 1.4.0 — coincidentally Ruling 3's N3 target transport — and the spike suite is green there too, a free early signal for the N3 reconciliation.
...
- **Session caveats:** (1) ~~the final confirmatory suite run could not be executed~~ **RESOLVED** — the wedge was the API Updater modal (declined); the confirmatory run then went green (4/4 + 1 ignored, job `fe3d5a82`) on the restored manifest (transport 1.5.0 / collections 1.4.0, NGO = fork spike branch). (2) Mid-session, MCP tool calls silently rerouted to the wrong Unity instance after bridge reconnects (lost pin); all evidence runs above were made with routing re-verified (`Application.dataPath` asserted) — treat any future "test job failed to initialize" as a routing symptom first.
